...
class subsub_SlotAttention(nn.Module):
    ...
    def forward(self, inputs, q_inp, k_inp, v_inp): # [B, HW, C]
        ...
        prompt_feature = torch.einsum('bnd,bnp->bdp', prompts, attn) # [B, dim, HW]]
        slot_feature = torch.einsum('bnd,bnp->bdp', slots, attn) # [B, dim, HW]]
        return slots, prompts, prompt_feature.permute(0, 2, 1), slot_feature.permute(0, 2, 1), attn
...
class sub_SlotAttention(nn.Module):
    ...
    def forward(self, inputs, q_inp, k_inp, v_inp): # [B, HW, C]
        ...
        for _ in range(self.iters):
            ...
            k = k * sub2_promptfeature
            v = v * sub2_promptfeature

        prompt_feature = torch.einsum('bnd,bnp->bdp', prompts, attn) # [B, dim, HW]]
        slot_feature = torch.einsum('bnd,bnp->bdp', slots, attn) # [B, dim, HW]]

        slot_cossim = [prompt_feature.permute(0, 2, 1), sub2_promptfeature, slot_feature.permute(0, 2, 1), sub2_slotfeature]

        attn = torch.cat([attn, sub2_attn], dim=1)

        return slots, prompts, prompt_feature.permute(0, 2, 1) * sub2_promptfeature, slot_feature.permute(0, 2, 1) * sub2_slotfeature, attn, slot_cossim
...
class Slot_in_slot_Attention(nn.Module):
    def __init__(self, num_slots, num_subslots, num_subsubslots, dim, iters = 3, eps = 1e-8, hidden_dim = 128):
        ...
        self.num_slots = num_slots
        self.num_subslots = num_subslots
        self.num_subsubslots = num_subsubslots
        self.iters = iters
        self.eps = eps
        self.scale = dim ** -0.5
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, inputs, q_inp, k_inp, v_inp): # [B, HW, C]
        ...
        slot_cossim_total = v * prompt_feature.permute(0, 2, 1) #* sub_promptfeature
        attn_total = torch.cat([attn, sub_attn], dim=1)
...
class IG_MSA(nn.Module):
    ...
    def forward(self, x_in):
        ...
        k_inp = self.to_k(x)
        v_inp = self.to_v(x)
        # print(q_inp.shape, k_inp.shape, v_inp.shape) # [8, 4096, 128*heads] [8, 4096, 128*heads] [8, 4096, 128*heads]
        ...
        for i in range(len(self.slot_list)):
            ...
            slots_cossim_list.append(slot_cossim_total)
...
class Slot_model(nn.Module):
    ...
    def __init__(self, cfg, use_slot=True):
        ...
        self.slot_num = 3
        self.subslot_num = 7
        self.subsubslot_num = 10
        self.TransformerBlock = IGAB(dim=self.dim*4, num_blocks=1, dim_head=self.dim*4, num_slots1=self.slot_num, num_slots2=self.subslot_num, num_slots3=self.subsubslot_num, heads=1)
        self.slot_decoder = Slot_Decoder(hid_dim=self.dim*4)

    def forward(self, x, gt, inference=True):
        ...
        x = self.activation(self.downsample1(conv1))
        conv2 = self.batchnorm2(self.conv2_2(x))

        # bottleneck
        x = self.activation(self.downsample2(conv2))
        conv3 = self.batchnorm3(self.conv3_2(x))
        ...
        conv4 = self.activation(self.conv4_1(up4))
        conv4 = self.activation(self.conv4_2(conv4))
        
        conv5 = self.upsample2(conv4)
        up5 = torch.cat([conv5, conv1], 1)
        conv5 = self.activation(self.conv5_1(up5))
        conv5 = self.activation(self.conv5_2(conv5))
            
        # output residual
        output = self.conv6(conv5) + x_input
